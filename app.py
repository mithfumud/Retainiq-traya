...
.sb-mark {
    ...
    font-size: 0.8rem; box-shadow: 0 2px 6px rgba(0,0,0,0.12);
}
.sb-name    { font-size: 0.78rem; font-weight: 700; color: #FFFFFF; letter-spacing: -0.2px; }
.sb-sub     { font-size: 0.55rem;  font-weight: 500; color: rgba(255,255,255,0.7); margin-top: 1px; }
...
.sb-stat-val { font-size: 0.72rem; font-weight: 700; color: #FFFFFF; letter-spacing: -0.3px; }
.sb-stat-lbl { font-size: 0.52rem; color: rgba(255,255,255,0.65); margin-top: 1px; }

/* ════════════════════════════════
   GLOBAL TOPBAR (full-width, above sidebar)
════════════════════════════════ */
.g-topbar {
...
.b-red     { background: rgba(239,68,68,0.12);    color: #FCA5A5;                border: 1px solid rgba(239,68,68,0.2); }
.b-yellow  { background: rgba(245,158,11,0.12);   color: #FCD34D;                border: 1px solid rgba(245,158,11,0.2); }
...
.rc-num   { font-size: 1.15rem; font-weight: 800; letter-spacing: -0.6px; line-height: 1; }
...
# ── Chart helper ──────────────────────────────────────────────────────────────
AXIS_TICK = dict(size=10, color="#0F172A", family="Inter, system-ui, sans-serif", weight=700)
AXIS_TITLE = dict(size=10, color="#0F172A", family="Inter, system-ui, sans-serif", weight=700)
...
def style_chart(fig, h=140, hbar=False):
    ...
    fig.update_layout(
        ...
        font=dict(family="Inter,system-ui,sans-serif", color="#0F172A", size=10),
        margin=dict(l=left_margin, r=12, t=8, b=40), height=h,
        hoverlabel=dict(bgcolor="#0F172A", font_color="#F8FAFC", font_size=10, font_family="Inter"),
    )
...
with st.sidebar:
    ...
    <div class="sb-brand">
      <div class="sb-logo-row">
        ...
        <div>
          <div class="sb-name">Traya Health</div>
          <div class="sb-sub">RetainIQ Platform</div>
        </div>
      </div>
    </div>
...
# ════════════════════════════════════════════════════════════════════════════
    ...
    hfig = go.Figure(go.Heatmap(
        ...
        texttemplate="<b>%{z}%</b>", showscale=True,
    ...
    hfig.update_layout(
        ...
        hoverlabel=dict(bgcolor="#0F172A",font_color="#fff",font_size=11),
    )
    ...
    est_lift = round(avg_m3*1.10,1)
    col_a, _ = st.columns([1,1])
    with col_a:
        st.markdown(f'<div class="bn warn"><b>{worst_m3}</b> has the lowest M3 retention at <b>{cp[3].min()}%</b>. Review onboarding quality, coach-call timing, and D7 check-in cadence for this cohort.</div>', unsafe_allow_html=True)
        st.markdown(f'<div class="bn info"><b>Recommendation:</b> Automated D7 + D14 coach check-ins estimated to lift avg M3 retention from {avg_m3}% → {est_lift}% (+10%).</div>', unsafe_allow_html=True)
...
# ════════════════════════════════════════════════════════════════════════════
    ...
    donut.update_traces(textfont_size=11, textfont_family="Inter")
    donut.update_layout(showlegend=True, legend=dict(orientation="h",y=-0.12,font=dict(size=10)))
...
# ════════════════════════════════════════════════════════════════════════════
    ...
    k5.metric("Premium–Basic Gap",     f"+{ltv_gap} mo")
